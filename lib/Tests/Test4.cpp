...
bool TestModule4::runOnModule(Module &M) {
  ...
  BranchInst::Create(mainExit, mainLeft);

  // synthesise main's right block
  BranchInst::Create(mainExit, mainRight);
...
static RegisterPass<TestModule4> X("test-module-decider",
                                   "Generate Test Module 4", false, false);
